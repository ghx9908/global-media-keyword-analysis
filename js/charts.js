let twoComboChart = null;
let threeComboChart = null;

function updateTwoComboChart() {
    if (!currentData || !currentData.two_combos) return;
    
    const ctx = document.getElementById('twoComboChart');
    if (!ctx) return;
    
    // 按count排序，取前10
    const sorted = [...currentData.two_combos]
        .sort((a, b) => b.count - a.count)
        .slice(0, 10);
    
    if (sorted.length === 0) {
        if (twoComboChart) {
            twoComboChart.destroy();
            twoComboChart = null;
        }
        return;
    }
    ...
    const labels = sorted.map(item => {
        const k1 = item.keyword1.length > 15 ? item.keyword1.substring(0, 15) + '...' : item.keyword1;
        const k2 = item.keyword2.length > 15 ? item.keyword2.substring(0, 15) + '...' : item.keyword2;
        return `${k1} + ${k2}`;
    });
